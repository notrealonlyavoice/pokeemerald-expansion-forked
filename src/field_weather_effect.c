...
const u16 gCloudsLeafallPalette[] = INCBIN_U16("graphics/weather/leaf.gbapal");
...
const u8 gWeatherLeafall1Tiles[] = INCBIN_U8("graphics/weather/leaf0.4bpp");
const u8 gWeatherLeafall2Tiles[] = INCBIN_U8("graphics/weather/leaf1.4bpp");
...
static void UpdateLeafallLeafSprite(struct Sprite *);
static bool8 UpdateVisibleLeafallLeafSprites(void);
static bool8 CreateLeafallLeafSprite(void);
static bool8 DestroyLeafallLeafSprite(void);
static void InitLeafallLeafSpriteMovement(struct Sprite *);

void Leafall_InitVars(void)
{
    gWeatherPtr->initStep = 0;
    gWeatherPtr->weatherGfxLoaded = FALSE;
    gWeatherPtr->targetColorMapIndex = 0;
    gWeatherPtr->colorMapStepDelay = 20;
    gWeatherPtr->targetLeafallLeafSpriteCount = NUM_LeafallLeaf_SPRITES;
    gWeatherPtr->LeafallLeafVisibleCounter = 0;
}

void Leafall_InitAll(void)
{
    u16 i;
    ...
    {
        Leafall_Main();
        for (i = 0; i < gWeatherPtr->LeafallLeafSpriteCount; i++)
            UpdateLeafallLeafSprite(gWeatherPtr->sprites.s1.LeafallLeafSprites[i]);
    }
}

void Leafall_Main(void)
{
    if (gWeatherPtr->initStep == 0 && !UpdateVisibleLeafallLeafSprites())
    {
        gWeatherPtr->weatherGfxLoaded = TRUE;
        gWeatherPtr->initStep++;
    }
...
bool8 Leafall_Finish(void)
{
    switch (gWeatherPtr->finishStep)
    {
    case 0:
        gWeatherPtr->targetLeafallLeafSpriteCount = 0;
        gWeatherPtr->LeafallLeafVisibleCounter = 0;
        gWeatherPtr->finishStep++;
        // fall through
    case 1:
        if (!UpdateVisibleLeafallLeafSprites())
        {
            gWeatherPtr->finishStep++;
            return FALSE;
        }
...
// LINE OF PLEASE DONT MFING TOUCH
static bool8 UpdateVisibleLeafallLeafSprites(void)
{
    if (gWeatherPtr->LeafallLeafSpriteCount == gWeatherPtr->targetLeafallLeafSpriteCount)
        return FALSE;

    if (++gWeatherPtr->LeafallLeafVisibleCounter > 36)
    {
        gWeatherPtr->LeafallLeafVisibleCounter = 0;
        if (gWeatherPtr->LeafallLeafSpriteCount < gWeatherPtr->targetLeafallLeafSpriteCount)
            CreateLeafallLeafSprite();
        else
            DestroyLeafallLeafSprite();
    }

    return gWeatherPtr->LeafallLeafSpriteCount != gWeatherPtr->targetLeafallLeafSpriteCount;
}

static const struct OamData sLeafallLeafSpriteOamData =
{
    .y = 0,
    .affineMode = ST_OAM_AFFINE_OFF,
    .objMode = ST_OAM_OBJ_NORMAL,
...
static const struct SpriteFrameImage sLeafallLeafSpriteImages[] =
{
    {gWeatherLeafall1Tiles, sizeof(gWeatherLeafall1Tiles)},
    {gWeatherLeafall2Tiles, sizeof(gWeatherLeafall2Tiles)},
};

static const union AnimCmd sLeafallLeafAnimCmd0[] =
{
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_END,
};

static const union AnimCmd sLeafallLeafAnimCmd1[] =
{
    ANIMCMD_FRAME(1, 16),
    ANIMCMD_END,
};

static const union AnimCmd *const sLeafallLeafAnimCmds[] =
{
    sLeafallLeafAnimCmd0,
    sLeafallLeafAnimCmd1,
};

static const struct SpriteTemplate sLeafallLeafSpriteTemplate =
{
    .tileTag = TAG_NONE,
    .paletteTag = PALTAG_WEATHER,
    .oam = &sLeafallLeafSpriteOamData,
    .anims = sLeafallLeafAnimCmds,
    .images = sLeafallLeafSpriteImages,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = UpdateLeafallLeafSprite,
};
...
static bool8 CreateLeafallLeafSprite(void)
{
    LoadCustomWeatherSpritePalette(gCloudsLeafallPalette);
    u8 spriteId = CreateSpriteAtEnd(&sLeafallLeafSpriteTemplate, 0, 0, 78);
    if (spriteId == MAX_SPRITES)
        return FALSE;

    gSprites[spriteId].tLeafallLeafId = gWeatherPtr->LeafallLeafSpriteCount;
    InitLeafallLeafSpriteMovement(&gSprites[spriteId]);
    gSprites[spriteId].coordOffsetEnabled = TRUE;
    gWeatherPtr->sprites.s1.LeafallLeafSprites[gWeatherPtr->LeafallLeafSpriteCount++] = &gSprites[spriteId];
    return TRUE;
}

static bool8 DestroyLeafallLeafSprite(void)
{
    if (gWeatherPtr->LeafallLeafSpriteCount)
    {
        DestroySprite(gWeatherPtr->sprites.s1.LeafallLeafSprites[--gWeatherPtr->LeafallLeafSpriteCount]);
        return TRUE;
    }
...
static void InitLeafallLeafSpriteMovement(struct Sprite *sprite)
{
    u16 rand;
    u16 x = ((sprite->tLeafallLeafId * 5) & 7) * 30 + (Random() % 30);
...
static void UNUSED WaitLeafallLeafSprite(struct Sprite *sprite)
{
    if (++gWeatherPtr->LeafallLeafTimer > 18)
    {
        sprite->invisible = FALSE;
        sprite->callback = UpdateLeafallLeafSprite;
        sprite->y = 250 - (gSpriteCoordOffsetY + sprite->centerToCornerVecY);
        sprite->tPosY = sprite->y * 128;
        gWeatherPtr->LeafallLeafTimer = 0;
    }
}

static void UpdateLeafallLeafSprite(struct Sprite *sprite)
{
    s16 x;
...
static u8 TranslateWeatherNum(u8 weather)
{
    ...
    case WEATHER_LEAFALL:            return WEATHER_LEAFALL;
